...
- **`OFFSET` is currently `0` in `config.h`** — a placeholder. Phase 1 will replace it with a runtime value (the base of an `mmap`'d heap), since on Linux ASLR won't put the heap at address 0.
- **`lispval` size doubles on x86_64.** `struct dtpr` was 8 bytes (pair of 32-bit pointers); now 16. Every `*SPP` constant in `global.h` needs reconsidering.
