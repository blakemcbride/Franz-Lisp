...
/* N.B. mytemp will return a pointer to static data that's overwritten
   on each call, that classic bug.  Besides, the length might be too small,
   now that we getenv("TMPDIR").  Lose, lose.  jd 17 oct 93 */

static char mybuff[256]; 
...
mytemp()
{
	/*if(mypid==0) mypid = (getpid() & 0xffff);
	  ...
	  cfasl */
	sprintf(mybuff,"%s/Li%d.%d",tmpdir(),(getpid() & 0xffff),seed++);
	return(mybuff);
}
...
{
	seed--;
	sprintf(mybuff,"%s/Li%d.%d",tmpdir(),(getpid() & 0xffff),seed-1);
	if(seed==0) {
		stabf = 0;
		fvirgin = 1;
	}
