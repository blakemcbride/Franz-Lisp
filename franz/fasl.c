...
/* /\/ For the compile-to-C compiler, most of the file has been sliced out
 *     temporarily.
 */
...
lispval Lfasl()
{
	return(error("FASL no yet available.",FALSE));
}

#else 		/* ------ sliced out when no FASL ------ */
...
#endif 		/* ------ end of stuff sliced out for no FASL ------ */
